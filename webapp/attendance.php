...
if (isset($_GET['delid'])) {
    ...
    $rid = $_GET['delid'];
    if ($rid != "") {
        echo "<script>alert('Data deleted' $rid );</script>";
        $strfidno ="";
        $stStudentID="";
        $stcourseid="";
        $sql = mysqli_query($con, "SELECT a.id,a.RFIDNumber,b.StudentID,c.courseName,b.EmailAddress,a.courseId from tblstdenroll a 
inner join tblstudents b on a.RFIDNumber=b.RFIDNumber inner join tblcourse c on a.courseId=c.id  where a.RFIDNumber=$rid");
        $row = mysqli_num_rows($sql);
        if ($row > 0) {
            while ($row = mysqli_fetch_array($sql)) :
                $strfidno = $row['RFIDNumber'];
                $stStudentID = $row['id'];
                $stcourseid = $row['courseId'];
                echo "<script>alert('Data Available');</script>";
            ...
            echo "$courseid - $studentid -  $studentrfid -";


            $sql = "INSERT INTO tblattendance(attdate,courseid,studentid,studentrfid) VALUES(?,?,?,?)";
            $statement = $con->prepare($sql);
            $statement->bind_param('ssss', $attdate, $courseid, $studentid, $studentrfid);
    ...
    <div class="attendanceProcess">
        ...
        <div class="payment-attendP">
            ...
            <div class="attendPermission">
                <a class="button" href="attendance.php?delid=<?php echo($rfidno); ?>" class="notiBtn" title="Delete"
                   data-toggle="tooltip">Attend Allowed</a>
                <a class="button button3" href="attendance.php?delid=<?php echo($rfidno); ?>" class="notiBtn"
                   title="Delete" data-toggle="tooltip">Attend Deny</a>
            </div>
        </div>
    </div>
</section>
