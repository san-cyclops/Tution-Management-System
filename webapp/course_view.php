...
$search = $_GET['search'] ?? '';
if ($search) {
  $statement = $dbh->prepare("SELECT * FROM tblcourse WHERE courseName LIKE '%$search%' OR LectureId LIKE '%$search%'");
} else {
  $statement = $dbh->prepare('SELECT * FROM tblcourse');
}
$statement->execute();
              ...
              <div class="col-sm-4">
                <form action="">
                  <div class="input-group mb-4">
                    <input type="text" class="form-control" placeholder="Search Courses" value="<?php echo $search ?>" name="search">
            ...
            <tbody>
              ...
              ?>
                ...
                <tr>
                  ...
                  <td><?php echo $course['LectureId']; ?></td>
                  <td><?php echo $course['cDay']; ?></td>
                  <td> <?php echo $course['ctimeE']; ?></td>
                  <td> <img style="width: 80px;background-size:100% 100%;" src="<?php echo $course['coursePic'] ?>" name="Pic" /></td>
                  <td>
                    <a href="course_edit.php?editid=<?php echo htmlentities($course['ID']); ?>" class="edit" title="Edit" data-toggle="tooltip"><i class="material-icons">&#xE254;</i></a>
                    <a href="course_view.php?delid=<?php echo ($course['ID']); ?>" class="delete" title="Delete" data-toggle="tooltip" onclick="return confirm('Do you really want to Delete ?');"><i class="material-icons">&#xE872;</i></a>
                  </td>
                </tr>
              <?php } ?>
